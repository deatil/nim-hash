...
proc p0(x: uint32): uint32 {.inline.} =
  return x xor rotl32(x, 9) xor rotl32(x, 17)

proc p1(x: uint32): uint32 {.inline.} =
  return x xor rotl32(x, 15) xor rotl32(x, 23)

proc ff(x, y, z: uint32): uint32 {.inline.} =
  return (x and y) or (x and z) or (y and z)

proc gg(x, y, z: uint32): uint32 {.inline.} =
...
proc secureHash*(str: openArray[char]): SM3SecureHash =
  runnableExamples:
    let hash = secureHash("abc")
    assert hash == parseSecureHash("66C7F0F462EEEDD9D1F2D46BDC10E4E24167C4875CF2F7A2297DA02B8F4BA8E0")

  var state = newSM3State()
  state.update(str)
  SM3SecureHash(state.finalize())

proc secureHashFile*(filename: string): SM3SecureHash =
...
proc `$`*(self: SM3SecureHash): string =
  runnableExamples:
    let hash = secureHash("abc")
    assert $hash == "66C7F0F462EEEDD9D1F2D46BDC10E4E24167C4875CF2F7A2297DA02B8F4BA8E0"

  result = ""
  for v in SM3Digest(self):
    result.add(toHex(int(v), 2))

proc parseSecureHash*(hash: string): SM3SecureHash =
  runnableExamples:
    let
      hashStr = "66C7F0F462EEEDD9D1F2D46BDC10E4E24167C4875CF2F7A2297DA02B8F4BA8E0"
      secureHash = secureHash("abc")
